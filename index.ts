...
async function run() {
  ...
  const dataset = (new tfd.CSVDataset(
                       new tfd.FileDataSource(localFileBuffer), {hasHeader: true,
                       columnConfigs: {'rings\r': {isLabel: true}}}))
                      .shuffle(10);
  ...
  const result = model.evaluate(
                     ...
                     tf.tensor1d([9, 9, 14])) as tf.Tensor;
